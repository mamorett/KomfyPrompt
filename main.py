...
class DropFrame(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        layout = QVBoxLayout()
        self.label = QLabel("Drag & Drop PNG/JSON file(s) or folder here")
    ...
    def dragEnterEvent(self, event: QDragEnterEvent):
        if event.mimeData().hasUrls():
            ...
            self.label.setText("Drop PNG/JSON file(s) or folder here!")
            self.label.setStyleSheet("color: #006600; font-size: 11pt; font-weight: bold;")
    
    def dragLeaveEvent(self, event):
        self.setStyleSheet("")
        self.label.setText("Drag & Drop PNG/JSON file(s) or folder here")
        self.label.setStyleSheet("color: gray; font-size: 11pt;")
...
class ComfyUIPromptExtractorUI(QMainWindow):
    ...
    def browse_file(self):
        file_paths, _ = QFileDialog.getOpenFileNames(
            self.browse_file_btn, # Note: self is passed as parent, not button
            "Select ComfyUI Files",
            "",
            "ComfyUI Files (*.png *.json);;PNG files (*.png);;JSON files (*.json);;All files (*.*)"
        )
        if file_paths:
    ...
    def browse_folder(self):
        folder_path = QFileDialog.getExistingDirectory(
            self,
            "Select Folder with ComfyUI Files"
        )
        if folder_path:
            ...
            if all_files:
                self.load_files(all_files)
            else:
                QMessageBox.information(self, "No Files", "No PNG or JSON files found in the selected folder.")
    
    def load_files(self, file_paths):
        # Filter for PNG and JSON files
        valid_files = []
        for file_path in file_paths:
            if os.path.isdir(file_path):
                ...
                valid_files.extend(json_files)
            elif os.path.exists(file_path):
                lower_path = file_path.lower()
                if lower_path.endswith('.png') or lower_path.endswith('.json'):
                    valid_files.append(file_path)
        
        if not valid_files:
            QMessageBox.warning(self, "Warning", "No valid PNG or JSON files found")
            return
    ...
    def process_files(self, file_paths):
        ...
        if len(file_paths) == 1:
            ...
            self.thumbnail_group.hide()
            self.thumbnail_info_label.setText(f"{len(file_paths)} files selected")
